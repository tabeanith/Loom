...
if __name__ == "__main__":
    ...
    for topic in [topic2, topic3]:
        ask_and_save_answer(TheGuardian(), topic, pd.Timestamp.now(tz=tz) - Day(days_back_in_time), use_ai=use_ai)
        ask_and_save_answer(Reuters(), topic, pd.Timestamp.now(tz=tz) - Day(days_back_in_time), use_ai=use_ai)
        ask_and_save_answer(SevereWeatherEurope(), topic, pd.Timestamp.now(tz=tz) - Day(days_back_in_time), use_ai=use_ai)

    # US Stocks
    for topic in [topic11]:
        ask_and_save_answer(Reuters(), topic, pd.Timestamp.now(tz=tz) - Day(days_back_in_time), use_ai=True)
